...
const calculateNetsalary = ()=>{
    ...
    $("#gross_salary").val((+$("#basicsalary").val() + +$("#da").val() + +$("#hra").val() + +$("#transport").val() + +$("#misc").val() + +$addAllowance));
    $("#total_deduction").val(+$("#pf").val() + +$("#npse").val() + +$addDeduction + +$addls + +$("#it").val());
    $("#net_salary").val((+$("#basicsalary").val() + +$("#da").val() + +$("#hra").val() + +$("#transport").val() + +$("#misc").val() + +$addAllowance) - (+$("#pf").val() + +$("#npse").val() + +$addDeduction + +$addls + +$("#it").val()));
}
...
$(document).ready((e)=>{
    ...
    $("#basicsalary").keyup((e)=>{
        // allowances
        $("#da").val(da_allowances($(e.target).val()));
        $("#hra").val(hra_allowances($(e.target).val()));
        $("#transport").val(travel_allowances($(e.target).val()));

        //deductions
        if(!$modFlag){
            $("#pf").val(pf_deduction($(e.target).val()));
        }
        $("#eps").val(eps_deduction($(e.target).val()));
        $("#epf").val(epf_deduction($(e.target).val()));
        $("#npse").val(nps_employee($(e.target).val()));
        $("#npser").val(nps_employer($(e.target).val()));
    ...
    $("#da").keyup((e)=>{
        if(!$modFlag){
            $("#pf").val(pf_deduction($("#basicsalary").val()));
        }
        // $("#eps").val(eps_deduction($(e.target).val()));
        $("#npse").val(nps_employee($("#basicsalary").val()));
        $("#npser").val(nps_employer($("#basicsalary").val()));

        calculateNetsalary();
    })
    $("#slab").keyup((e)=>{
        $("#transport").val(travel_allowances());
        calculateNetsalary();
    })
    $("#misc,#hra,#transport,#pf,#npse,#it").keyup((e)=>{
        calculateNetsalary();
    })
    $("#pf").keyup(()=>{
        let pfAmnt = $("#pf").val();
    ...
    $("#submitForm").click((e)=>{
        ...
        data=[
            ...
            {"basicsalary":"required|numeric|msg:Basic salary is required"},
            {"da":"required|numeric|msg:DA allowances required"},
            {"hra":"required|numeric|msg:HRA allowances required"},
            {"transport":"required|numeric|msg:Transport allowances required"},
            {"gross_salary":"required|numeric|msg:Gross Salary is required"},
            {"pf":"required|numeric|msg:PF is required"},
            {"npse":"required|numeric|msg:NPS-Employee required"},
            {"npser":"required|numeric|msg:NPS-Employer required"},
            {"net_salary":"required|numeric|msg:Net Salary is required"},
        ];
        if($(".allowance_card").length>0){
            $('.allowance_card').each(function() {
