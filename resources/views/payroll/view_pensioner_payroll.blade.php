...
<div class="employees-list shadow-md p-2 rounded bg-white">
    <div class="d-flex justify-content-between align-items-center p-2 rounded">
        <a href="{{route('pensioner_payroll')}}" class="btn btn-light d-flex gap-10 fw-500 text-danger rounded btn-sm" style="font-weight: bold"><i class="mdi mdi-chevron-left" style="font-size:15px"></i><span>Back</span></a>
        <div>  
            <a href="#" onclick="verifyPayroll(this.href);return false;" class="btn btn-inverse-success btn-sm mr-2 rounded verifyBtn" data-toggle="tooltip" data-placement="bottom" title="Click to verify" style="font-weight: bold"><i class="mdi mdi-check" style="vertical-align: middle;"></i> <span>Verify</span></a>
                ...
                @endif -->
            <div class="btn-group">
                <a href="{{route('pensioner_payroll.verify_payroll_prev',['id'=>Crypt::encryptString($structure->employee),'month'=>$month])}}" class="btn btn-outline-secondary btn-sm d-flex items-center justify-center" data-toggle="tooltip" data-placement="bottom" title="Previous"><i class="mdi mdi-chevron-left" style="font-size:15px"></i><span>Prev</span></a>
                ...
                <a href="{{route('salary-structure.modify_pension_structure',['month'=>date('Y-m',strtotime($month)),'id'=>Crypt::encryptString($structure->id),'route'=>Request::url()])}}" class="btn btn-outline-info btn-sm d-flex items-center justify-center" data-toggle="tooltip" data-placement="bottom" title="Edit"><i class="mdi mdi-pencil"></i></a>
                <a href="{{route('pensioner_payroll.verify_payroll_next',['id'=>Crypt::encryptString($structure->employee),'month'=>$month])}}" class="btn btn-outline-secondary btn-sm d-flex items-center justify-center" data-toggle="tooltip" data-placement="bottom" title="Next"><span>Next</span><i class="mdi mdi-chevron-right" style="font-size:15px"></i></a>
            </div>
        </div>
    </div>
    @if (session()->has("status"))
        ...
        @endif
    @endif
    <form action="{{route('pensioner_payroll.verify',['id'=>Crypt::encryptString($structure->employee),'month'=>$month])}}" id="payrollVerify" method="POST">
    @csrf
    <div id="DivIdToPrint">
        ...
        <table class="table table-bordered mt-2" id="employeesTable">
            ...
            <th class="text-center  bg-gray-200">Category</th>
            <th class="text-center  bg-gray-200">DOR</th>
            <tr>
                <td class="text-center">{{strtoupper($structure->employee)}}-{{ucwords($structure->empname)}}</td>
                <td class="text-center text-capitalize">{{$structure->category}}</td>
                <td class="text-center">{{date("d-M-Y",strtotime($structure->empdor))}}</td>
            </tr>
        </table>
        ...
        <table class="table table-bordered mt-4" id="employeesTable">
            <tbody>
                <tr>
                    ...
                    <tr>
                        <th class="text-left" colspan="4">Basic Pension</th>
                        <td class="text-center">{{number_format($structure->basic_salary)}}</td>
                    </tr>
                    <tr>
                        <th class="text-left" colspan="4">Additional Pension</th>
                    ...
                    <tr>
                        <th class="text-left" colspan="4">Medical Allowance</th>
                    ...
                    <tr>
                        <th class="text-left" colspan="4">Less Commutation</th>
                ...
                @endif
                <tr>
                    <th class="text-left" colspan="4">Gross Total</th>
                ...
                <tr>
                    ...
                    <tr>
                        <th class="text-left" colspan="4">Misc. Recovery</th>
                        <td class="text-center">{{number_format($structure->misc_rec)}}</td>
                    </tr>
                    <tr>
                        <th class="text-left" colspan="4">IRG</th>
                ...
                @endif
                    ...
                    <tr>
                        <th class="text-left" colspan="4">Net Pension</th>
                ...
                @endforeach
                    ...
                    <tr>
                        ...
                        <th class="text-center">{{number_format($structure->net_salary+$total_da)}}</th>
                    </tr>
                @endif
                    <input type="hidden" id="total_salary" name="total_salary" value="{{$structure->net_salary+$total_da}}">
            </tbody>
        </table>
</div>
</form>
