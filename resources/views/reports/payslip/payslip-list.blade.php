...
@section('content')
    ...
    <div class="employees-list shadow-md p-2 rounded bg-white">
        <div class="d-flex justify-content-between align-items-center p-2 rounded">
            <h4 class="m-0">Payslip List</h4>
            <div>
                <a href="{{ route('employees.export_employees') }}" class="btn btn-dark "><i class="icon-file mr-2"></i>Export
                    Excel</a>
        ...
        <div class="form-group col-md-4">
            <label for="month">Select month to view Payslip</label>
            <form action="{{ route('payroll.getMonth_payslip') }}" method="POST" id="getMonth">
                @csrf
                <div class="input-group mb-3">
                    <input type="text" id="month" name="month" class="form-control" aria-describedby="basic-addon2"
                        value="{{ $month }}" readonly required placeholder="MM/YY">
                                ...
                                @foreach($employees as $emp)
                                    @if(strtolower($emp->category) == strtolower($category->category))
                                        <tr class="bg-light">
                                            ...
                                            <td>
                                                <!-- <a href="{{route('payroll.verify_payroll',['id'=>Crypt::encryptString($emp->empid),'month'=>$month])}}" class="btn btn-sm btn-inverse-success" data-toggle="tooltip" data-placement="bottom" title="View Salary Structure"><i class="icon-eye menu-icon"></i></a> -->
                                                @if(in_array($emp->empid, $verifiedEmployees))
                                                    <a href="{{ route('payroll.getpayslip', ['id' => Crypt::encryptString($emp->empid), 'month' => $month]) }}" class="btn btn-sm btn-inverse-dark" data-toggle="tooltip" data-placement="bottom" title="Download Payslip"><i class="icon-file menu-icon"></i></a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endif
