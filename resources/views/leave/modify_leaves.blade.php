...
<div class="employees-list p-2 rounded bg-white">
    <div class="d-flex justify-content-between align-items-center p-2 rounded">
        <h4 class="m-0">Modify Leave Request</h4>
    ...
    @if (session()->has("status"))
        ...
        @endif
    @endif
    <form action="{{route("leaves.update_leaves",['id'=>Crypt::encryptString($leave->id)])}}" method="POST" id="attendanceForm">
        @csrf
                        ...
                        <select id="empid" name="empid" class="block w-full mt-1 rounded-md" required>
                            <option value="">---Select Employee ---</option>
                             @foreach ($employees as $employee)                            
                                <option value="{{ $employee->empid }}" @if(old('empid')==$employee->empid || $leave->empid == $employee->empid) selected @endif>{{ $employee->empname }} - {{ $employee->empid }}</option>
                            @endforeach
                        </select>
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select name="leavetype" id="leavetype" class="form-control" required>
                            ...
                            <option value="el" @if(old('leavetype')=='el'||$leave->leavetype == 'el' ) selected @endif>Earned Leave</option>
                            <option value="ml" @if(old('leavetype')=='ml'||$leave->leavetype == 'ml' ) selected @endif>Medical Leave</option>
                            <option value="cl" @if(old('leavetype')=='cl'||$leave->leavetype == 'cl' ) selected @endif>Casual Leave</option>   
                        </select>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control dtclass" id="startdt" name="startdt" placeholder="" value="{{ old('startdt')?old('startdt'):$leave->startdt }}" required>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control dtclass" id="enddt" name="enddt" placeholder="" value="{{ old('enddt')?old('enddt'):$leave->enddt }}" required>
            ...
            <div class="row">
                <div class="col-md-8">
                    <div class="form-group">
                        ...
                        <input type="text" id="remark" name="remark" class="form-control" placeholder="Remarks If Any" value="{{ old('remark')?old('remark'):$leave->remark }}">
                    ...
                    <div class="form-group">
                        <div id="fh">
                            ...
                            <select name="days" id="days" class="form-control" required>
                                ...
                                <option value="1" @if(old('days')=='1'|| $leave->days == '1') selected @endif>Full Day</option>
                                <option value="0.5" @if(old('days')=='0.5'|| $leave->days == '0.5') selected @endif>Half Day</option>
                            </select>
...
@if($leave->days=='1')
<script>$('#fh').show();</script>
@else
<script>$('#fh').hide();</script>
@endif
