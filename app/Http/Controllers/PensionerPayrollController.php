...
class PensionerPayrollController extends Controller
{
    public function index(Request $request){
        ...
        $payrollCount = payroll::join('salary_structure', 'payroll.employee', '=', 'salary_structure.employee')
                ...
                $TempCategories = SalaryStructure::where("pensioner",1)->pluck('category');
        ...
        // dd($payrollCount);
        $employees = SalaryStructure::leftjoin("designation","designation.id","=","salary_structure.designation")
        ...
        $verifiedEmployees = payroll::where('month', '=', $monthTemp)->where("pensioner",1)
                            ->pluck('employee')
                            ->toArray();
        return view("payroll.pensioner_payroll",["categories"=>$categories,"employees"=>$employees,"month"=>$month,"payrollCount"=>$payrollCount,"verifiedEmployees"=>$verifiedEmployees]);
    }
    ...
    public function verify_payroll(Request $request){
        ...
        if($curr_month == $sel_month){
            $structure = SalaryStructure::leftJoin("employees","employees.empid","=","salary_structure.employee")
                        ...
                        ->select("salary_structure.*","departments.department as dept","departments.desg_department as dept_desc","designation.designation as desg","designation.desg_description as desg_desc","employees.empname","employees.category","employees.empdor","employees.pf_nps_cat")
            ...
            $structure = payroll::leftJoin("employees","employees.empid","=","payroll.employee")
                        ...
                        ->select("payroll.*","departments.department as dept","departments.desg_department as dept_desc","designation.designation as desg","designation.desg_description as desg_desc","employees.empname","employees.category","employees.empdor","employees.pf_nps_cat")
        ...
        //DA arrear check end
        return view("payroll.view_pensioner_payroll",["structure"=>$structure,"allowances"=>$allowances,"deductions"=>$deductions,"la"=>$la,"back"=>"payroll","month"=>$month,"da_arrear"=>$da_arrear,"payrolls"=>$payrolls,"prev_da"=>$prevDA_perc,"verified"=>$verifiedFlag]);
    }
    ...
    public function verify_payroll_next(Request $request){
        ...
        $employees = SalaryStructure::where("status",0)->where("category",$curr_emp->category)->where("pensioner",1)->orderBy("employee","ASC")->pluck('employee')->toArray();    
    ...
    public function verify(Request $request){
        ...
        $payroll->addtl_pension = ($article->addtl_pension)?$article->addtl_pension:0;;
        ...
        $payroll->hra =0;
        $payroll->transport = 0;
        $payroll->medic_allow = ($article->medic_allow)?$article->medic_allow:0;
        $payroll->less_comm = ($article->less_comm)?$article->less_comm:0;
        ...
        $payroll->misc_rec = ($article->misc_rec)?$article->misc_rec:0;
        $payroll->irg = ($article->irg)?$article->irg:0;
    ...
    public function verify_department_payroll(Request $request){
        ...
        try {
            ...
            foreach($payrolls as $payroll){
                ...
                $payroll_final->department = "";
                $payroll_final->designation = "";
                ...
                $payroll_final->addtl_pension = $payroll->addtl_pension;
                $payroll_final->medic_allow = $payroll->medic_allow;
                $payroll_final->less_comm = $payroll->less_comm;
                $payroll_final->misc_rec = $payroll->misc_rec;
                $payroll_final->irg = $payroll->irg;
                $payroll_final->pensioner = 1;
                $payroll_final->created_by = Auth::user()->id;
                $payroll_final->save();
            }
            return redirect()->route("pensioner_payroll")->with(["status"=>true,"message"=>"Payroll Generated successfully! for ".ucwords($department)." Staffs","month"=>$monthFull]);
        } catch (Exception $e) {
            //throw $th;
            return back()->with(["status"=>false,"message"=>"Can't verify payroll right now!"]);
        }
